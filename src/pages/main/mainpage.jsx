...
const MainPage = ({ currentUser, loginState }) => {
  ...
  const originalEvent = loginState
    ? scheduleConverter({
            ...
            >
              {recentNotices.map((notice, index) => (
                ...
                >
                  ...
                  <span className="ml-2 flex-1 text-sm text-gray-800 truncate">
                    {notice.title}
                  </span>
...
const DeadlineNotice = () => {
  ...
  const sortedPrograms = [...programs]
    .sort(
      (a, b) =>
        a.now_capacity / a.max_capacity - b.now_capacity / b.max_capacity
    )
    .slice(0, 4);
        ...
        <div className="grid grid-cols-2 gap-3">
          {sortedJobs.map((job) => {
            const daysLeft = Math.ceil(
              (new Date(job.deadline) - currentDate) / (1000 * 60 * 60 * 24)
            );
            return (
              <div
                key={job.id}
        ...
        <div className="grid grid-cols-2 gap-3">
          {sortedPrograms.map((program) => {
            ...
            const capacityPercentage =
              (program.now_capacity / program.max_capacity) * 100;
            return (
              <div
                key={program.id}
                className="p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
              ...
              >
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium text-sm truncate">
                    {program.title.length > 13
                      ? program.title.substring(0, 11) + "..."
                      : program.title}
                  </span>
